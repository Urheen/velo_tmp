...
_registry = {
    ...
    FrozenDict: _RegistryEntry(lambda xs: unzip2(sorted(xs.items()))[::-1],
                         lambda keys, xs: dict(zip(keys, xs))),
    FlatMap:  _RegistryEntry(lambda xs: unzip2(sorted(xs.items()))[::-1],
                         lambda keys, xs: dict(zip(keys, xs))),
...
_register_keypaths(
    FlatMap, lambda x: tuple(DictKey(k) for k in x.keys())
)

_register_keypaths(
    FrozenDict, lambda x: tuple(DictKey(k) for k in x.keys())
)

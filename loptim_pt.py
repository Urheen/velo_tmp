...
def _build(params_path):

  def fn():
    return opt_from_checkpoint.opt_from_checkpoint(params_path)

  return fn

_lopt_fn = _build('./pretrained_lopts/aug12_continue_on_bigger_2xbs_200kstep_bigproblem_v2_5620/params')
...
@gin.configurable
class PartialHyperV2(lopt_base.LearnedOptimizer):
  ...
  @staticmethod
  def is_update_params(path, value, udps):
    for udp in udps:
      if udp in path:
        return 1
    return 0
  ...
  def opt_fn(self, theta, is_training=True) -> opt_base.Optimizer:
    ...
    class _Opt(opt_base.Optimizer):
      ...
      def init(self,
        ...
        # (zeyu): change for just updating a part of the parameters  
        is_udp = partial(parent.is_update_params, udps=parent.update_params)
        n_states = onp.sum(jax.tree_util.tree_leaves(
          tree_utils.map_named(
            # lambda k, v: 1 if 'linear_1' not in k else 0, 
            is_udp,
            params
            )
        ))
        lstm_hidden_state = jax.tree_util.tree_map(
            lambda x: jnp.tile(x, [n_states] + [1] * len(x.shape[1:])),   # 这里tile就是把最高维度复制n_state遍，n_state为需要更新的参数数目
      ...
      def update(self,
        ...
        grads = jax.tree_util.tree_map(lambda x: jnp.clip(x, -1000., 1000.), grads)
        ...
        # 这里通过函数ff把输入信息转化为rnn_input
        # rnn_iunputs: a flat map, each parameter corresponds to a vector
        # print(f"the rnn_inputs: {rnn_inputs}")
        rnn_inputs = jax.tree_util.tree_map(ff, opt_state.params, grads, m, rms,
                                            summary_prefix)

        # 这里把rnn_inputs转化为lstm的输入向量
        stack = jnp.asarray(jax.tree_util.tree_leaves(rnn_inputs))
        # (zeyu): construct a mask
        is_udp = partial(parent.is_update_params, udps=parent.update_params)
        stack_mask = jax.tree_util.tree_leaves(
          tree_utils.map_named(
          # lambda k, v: 1 if 'linear_1' not in k else 0, 
          is_udp,
          rnn_inputs
        ...
        _stack = []
        for s, sm in zip(stack, stack_mask):
          if sm == 1:
            _stack.append(s)
        _stack = jnp.asarray(_stack)
        lstm_hidden_state = opt_state.lstm_hidden_state
        # lstm_hidden_state: [num_param_tensor, 512]
        # print(f"the lstm hidden_state: {lstm_hidden_state}") 
        ...
        control_params, lr_mult, next_lstm_hidden_state = parent.rnn.apply(
            ...
            _stack, 
            lstm_hidden_state)
        # control_params: [num_param_trensor, 256]
        # print(f"the control_params: {control_params}")
        ...
        lr_mult = [lr for lr in lr_mult]
        cp, lm, j = [], [], 0
        for sm in stack_mask:
          if sm == 1:
            cp.append(control_params[j])
            lm.append(lr_mult[j])
            j += 1
          else:
            cp.append(jnp.zeros_like(control_params[0]))
            lm.append(jnp.zeros_like(lr_mult[0]))
        ...
        # 这里把每一个control_params分配原来的参数
        # control_params = struct.unflatten(control_params)
        control_params = struct.unflatten(cp)
        lr_mult = struct.unflatten(lm)
        ...
        def apply_one(paths, control_param, key, lr_mult, p, g, m, rms, fac_g, v_col,
                      v_row, v):
          path = '/'.join(p.key for p in paths)
          update = False
          for udp in parent.update_params:
            if udp in path:
              update = True
              break
          if not update:
        ...
        """
        from utils.mytree_utils import tree_map_with_path
